  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record ) => (
          <span>
            <a href="javascript:;" onClick={() => this.handleStatusChange(record.id,record.status == '正常' ? 2 : 1)}>{record.status == '正常' ? '禁用' : '启用'}</a>
            <Divider type="vertical" />
            <a href="javascript:;" onClick={() => this.showEditFormModal(record.id)}>编辑</a>
            <Divider type="vertical" />
            ...
            <Row type="flex" justify="start">
              ...
              <Col span={12}>
                <div className={styles.floatRight}>
                  <Button type="primary" onClick={() => this.showCreateFormModal()}><Icon type="plus-circle" />新增菜单</Button>
                  &nbsp;
              ...
              {getFieldDecorator('icon',{
                ...
                >
                  {this.state.data.icons.length !== 0
                    ? this.state.data.icons.map(d => <Option key={d}> <Icon type={d} /> {d}</Option>)
                    : console.log('select data null')}
                </Select>
              )}
