    ...
    const columns = [
      {
        ...
        key: 'title',
        width: 500,
        render: (text, record ) => <a href="javascript:;" onClick={() => this.handleEdit(record.id)}>{text}</a>,
      },
      {
        title: '排序',
        dataIndex: 'sort',
      ...
      },
      {
        ...
        key: '',
        render:(text,record) => {
          return <div style={{width:'100px',height:'30px',overflow:'hidden'}}>
          <a href="javascript:;" onClick={() => { this.setState({previewVisible:true,previewImage:record.path});}}><img src={text.cover_path} alt="" style={{width:'100%'}} /></a></div>
        }
      },
      {
        title: '状态',
      ...
      },
      {
        ...
        render: (text, record ) => (
          <span>
            <a href="javascript:;" onClick={() => this.handleStatusChange(record.id,record.status == '正常' ? 2 : 1)}>{record.status == '正常' ? '禁用' : '审核'}</a>
            ...
            <Row type="flex" justify="start">
              ...
              <Col span={12}>
                <div className={styles.floatRight}>
                  <Button type="primary" onClick={() => this.handleCreate()}><Icon type="plus-circle" />发布广告</Button>
                </div>
              </Col>
            </Row>
          </div>
