...
<body>
  <article class="aui-content">
      <div class="pay-status">
          ...
          <p class="aui-font-size-12 aui-padded-t-15 aui-margin-t-15">您已成功支付（港币）</p>
          <h1 style="font-size:34px;">{{$order['buyer_pay_amount']}}</h1>
      </div>
      <div class="aui-content-padded">
        <div class="aui-btn aui-btn-danger aui-btn-block aui-btn-sm">
            <a href="{{$url}}?order_id={{$order['id']}}" style="color:#fff">完成</a>
        </div>
      </div>
  </article>
    <script>
        setTimeout(function(){
            window.location.href="{{$url}}?order_id={{$order['id']}}";
        }, 2000);
    </script>
</body>
</html>