...
class CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('wechat/cart/index',compact('shoppingCarts'));
    }
    ...
    public function submit(Request $request)
    {
        ...
        if(!empty($user)) {
            if(!empty($cartData)) {
                ...
                $url = url('wechat/order/info?data='.json_encode($jsonData).'&cartIds='.json_encode($cartIds));
            } else {
                $url = 'http://'.$_SERVER['SERVER_NAME'].'/wechat/cart/index';
            }

        } else {
            // 未登录，跳转到登录界面
            $url = url('login?returnUrl=http://'.$_SERVER['SERVER_NAME'].'/wechat/cart/index');
        }
