...
class CreateGoodsOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('goods_orders', function (Blueprint $table) {
            ...
            $table->tinyInteger('close_type')->nullable()->comment('订单关闭类型：1，买家取消订单；2，卖家取消订单；3，超时关闭；');
            $table->tinyInteger('close_reason')->nullable()->comment('订单关闭原因：1，我不想买了；2，信息填写错误，重新拍；3，卖家缺货；4，付款遇到问题；5，拍错了；6，其他原因');
            ...
            $table->string('timeout_receipt')->nullable()->comment('该笔订单允许的最晚确认收货时间，逾期将自动确认收货。取值范围：1m～15d。m-分钟，h-小时，d-天，1c-当天（1c-当天的情况下，无论交易何时创建，都在0点关闭）。 该参数数值不接受小数点， 如 1.5h，可转换为 90m');
